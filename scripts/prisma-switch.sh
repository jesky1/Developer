...
if [[ "$DB_URL" == file:* ]]; then
  ...
  echo "[prisma-switch]    Defaulting to PostgreSQL schema (Vercel default)."
  if [ -f "$PG_SCHEMA" ]; then
    cp "$PG_SCHEMA" "$ACTIVE_SCHEMA"
  elif [ -f "$SQLITE_SCHEMA" ]; then
    cp "$SQLITE_SCHEMA" "$ACTIVE_SCHEMA"
  fi
fi
...
if [ ! -f "$ACTIVE_SCHEMA" ]; then
  echo "[prisma-switch] ⚠️  schema.prisma does not exist. Creating from schema.pg.prisma..."
  if [ -f "$PG_SCHEMA" ]; then
    cp "$PG_SCHEMA" "$ACTIVE_SCHEMA"
    echo "[prisma-switch] ✅ Created schema.prisma from schema.pg.prisma"
  else
    echo "[prisma-switch] ❌ Neither schema.prisma nor schema.pg.prisma exists!"
    exit 1
  fi
fi
